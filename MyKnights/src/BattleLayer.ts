enum BATTLE_TYPE
{
	GUIDE_BATTLE1 = -1, //第一场引导战斗
	UNKNOWN_BATTLE = 0, // 未知战斗
	TEST_BATTLE,        // 测试战斗
	QUEST_BATTLE,       // 任务战斗
	ARENA_BATTLE,       // 竞技场战斗
	JOB_TEAM_BATTLE,    // 转职副本战斗
}
...
enum BATTLE_STATUS
{
	MOVE, BE_REMOTE_HIT, BE_HIT, DEAD, BACKING, HIT
}

enum BULLET_TYPE
{
	BULLET_BOW = 1, BULLET_MAGIC, 
	BULLET_LONG_BOW, BULLET_CROSS_BOW, BULLET_FIRE, BULLET_ICE, BULLET_SAN,
}
...
class Bullet
{
	...
	public move_direction:number; //移动方向，1表示从左到右，-1相反

	public attack:number; // 攻击力，取整
	public move_velocity:number; //移动速度，取到小数点后两位
	public pos_x:number; //当前位置，取到小数点后两位

	public remote_hit_back_distance:number; //远程攻击击退敌人距离，取到小数点后两位

	// buff
	public self_critical_value:number;   // 暴击比例

	public self_frozen_value:number;   // 减速比例
	public self_frozen_time:number;    // 减速持续时间

	// Just for client.
	public bullet_layer:egret.DisplayObjectContainer = null;
	public bullet_dragon:dragonBones.Armature = null;

	public constructor() {
		this.isPlayer = false;

		this.type = BULLET_TYPE.BULLET_BOW;
		this.name = "???";
...
class Knight {
	...
	public type:KNIGHT_TYPE;
	public name:string;
	public level:number;

	public attack_factor:number; //攻击系数
	public defense_factor:number; //防御力系数
	public hp_factor:number; //生命值系数

	public move_velocity:number; //移动速度，取到小数点后两位
	public pos_x:number; //当前位置，取到小数点后两位

	public move_direction:number; //移动方向，1表示从左到右，-1相反
	public move_frozen_value:number; //自身被减速比例，0-100，100速度降低为0
	public move_frozen_end_time:number; //自身被减速结束时间

	public hp:number; //生命值，取整
	public hp_max:number; //生命值上限，取整
	public defense:number; //防御力，取整
	
	public hit_attack:number; //近程攻击力，取整
	public hit_show_time:number; //近程攻击前摇
	public hit_show_distance:number; //近程开始播放攻击动画的距离
	public hit_start_time:number; //近程攻击前摇动画开始时间
	public hit_back_distance:number; //近程攻击击退敌人距离，取到小数点后两位
	public hit_delay_damage:number; //自身的伤害累加值，用于BE_HIT时计算的伤害值，取整

	public remote_hit_warning_distance:number; //远程攻击的警戒距离，低于该距离变成近战

	public remote_hit_attack:number; //远程攻击力，取整
	public remote_hit_show_time:number; //远程攻击前摇
	public remote_hit_start_time:number; //远程攻击开始时间
	public remote_hit_combo:number; //远程攻击连续次数
	public remote_hit_distance:number; //远程攻击距离，取到小数点后两位
	public remote_hit_back_distance:number; //远程攻击击退敌人距离，取到小数点后两位
	public remote_hit_cd:number; //远程攻击冷却时间
	public remote_hit_cd_start_time:number; //远程攻击cd开始时间
	public remote_hit_move_velocity:number; //远程子弹移动速度

	public remote_recover_attack:number;   //远程治疗能力值，取整
	public remote_recover_distance:number; //远程治疗距离，取到小数点后两位

	public self_back_velocity:number; //自身击退速度，取到小数点后两位
	public self_back_distance:number; //自身剩余击退距离，取到小数点后两位
	public self_back_velocity_a:number; //自身击退加速度，取到小数点后两位
	public self_back_start_time:number; //自身击退开始时间

	public self_back_immune_invalid:boolean; //两个斗士相遇，击退免疫失效
	public self_back_immune_percent:number; //击退免疫概率，0-10000，10000为必触发
	public self_back_immune_value:number;   //击退免疫比例，0-100，100为全部免疫

	public self_sunder_armor_percent:number; //破甲概率，0-10000，10000为必触发
	public self_sunder_armor_value:number;   //破甲比例，0-100，100为忽视对手护甲

	public self_critical_percent:number; //暴击概率，0-10000，10000为必触发
	public self_critical_value:number;   //暴击比例，200为2倍暴击

	public self_frozen_percent:number; //减速概率，0-10000，10000为必触发
	public self_frozen_value:number;   //减速比例，0-100，100速度降低为0
	public self_frozen_time:number;    //减速持续时间

	public status:BATTLE_STATUS;

	// Just for client.
	public head_icon:egret.Bitmap;
	public hp_mask:egret.Rectangle;

	public knight_layer:egret.DisplayObjectContainer = null;
	public knight_dragon:dragonBones.Armature = null;
	public knight_shadow_dragon:dragonBones.Armature = null;

	public fire_dragon:dragonBones.Armature = null;
	public ice_dragon:dragonBones.Armature = null;
	public sunder_dragon:dragonBones.Armature = null;
	public recover_dragon:dragonBones.Armature = null;

	public constructor() {
		this.isPlayer = false;

		this.type = KNIGHT_TYPE.KNIGHT_SOLDIER;
		this.name = "???";
		this.level = 1;
		...
		this.status = BATTLE_STATUS.MOVE;
...
class Knight_BaseInfo {
	...
	public pos_offset:number; //位置偏移量，从战场边缘向中心偏移
	public move_direction:number; //移动方向，1表示从左到右，-1相反
	public left_hp:number; //战斗初始的hp，如果为-1则表示初始为hp_max
...
class BattleLayer extends egret.DisplayObjectContainer {
	public static battle_type:BATTLE_TYPE = BATTLE_TYPE.UNKNOWN_BATTLE;
	public static self_knight_direction:number = 1; //玩家自己骑士所在方位，1左，-1右
	public static is_server_battle:boolean = false;
	public static server_battle_result:number = 0; // 1: left win, 0: draw, -1:right win

	// BATTLE_TYPE.QUEST_BATTLE
	public static my_knights_left_hp = [null, null, null, null, null];
	public static city_id:number = 0;
	public static city_battle_index:number = 0;

	public static knights_baseinfo_list = [];

	public battle_result_dialog = null;
	public guiLayer:egret.gui.UIStage;

    public main_director:Main;

    private background:egret.Bitmap = null;
    private shadow_layer:egret.DisplayObjectContainer = null;
    private bottom_bg_layer:egret.DisplayObjectContainer = null;
    private bottom_knights_layer:egret.DisplayObjectContainer = null;
    private top_hp_layer:egret.DisplayObjectContainer = null;

    private SELF_KNIGHT_SMALL_HP_WIDTH:number = 112;
    private KNIGHT_TEAM_HP_WIDTH:number = 240;

    public battle_end_flag:boolean = false;
    public left_team_hp_anim_mask:egret.Rectangle;
    public right_team_hp_anim_mask:egret.Rectangle;

    private left_knights_list = [];
    private right_knights_list = [];

    private left_bullets_list = [];
    private right_bullets_list = [];

    private MAX_TIME:number = 60; // 战斗最多进行60s，到达60秒以后进行胜负判断
	private TIME:number = 0;
	private TICK_TIME:number = 0.05; // 0.05s是最小时间间隔，所有的时间相关的数据必须为0.05s的整数倍，比如攻击前摇、攻击冷却等
	
	private LEFT_SIDE:number = 0; //战场左边边缘坐标
	private RIGHT_SIDE:number = 20; //战场右边边缘坐标

	private GROUND_POS_Y:number = G.win_height/2 + 110; //战场地面Y坐标
	private GROUND_POS_MINX:number = 100; //战场最左X坐标
	private GROUND_POS_MAX:number = 1036; //战场最左X坐标
	private GROUND_POS_X_FACTOR:number = 46.8; //X坐标像素和米的比例因子

	private bottom_alive_knight_zorder = -1;

	public logic_timer:egret.Timer = null;

	private IS_DEBUG_OUTPUT:boolean = false;

    public constructor() {
        super();
    }

    public static KnightGoToBattle(knight_data:Knight_BaseInfo) {
    	BattleLayer.knights_baseinfo_list.push(knight_data);
    }
    public static ClearAllBattleBaseInfo() {
    ...
    public static MyKnightInLeft() {
    	BattleLayer.self_knight_direction = 1;
    }
    public static MyKnightInRight() {
    	BattleLayer.self_knight_direction = -1;
    }

    public static InitQuestBattleInfo(city_id:number) {
    	BattleLayer.battle_type = BATTLE_TYPE.QUEST_BATTLE;
    	BattleLayer.self_knight_direction = 1;
		BattleLayer.is_server_battle = false;
		BattleLayer.server_battle_result = 0;
    ...
    private static AddPlayerKnightsToBattleForQuest() {
    	for (var i = 0; i < 5; i++) {
    ...
    private static AddQuestNpcKnightsToBattle() {
    	var player_knight = Logic.getPlayerKnight();
    	var player_knight_type = KNIGHT_TYPE.KNIGHT_SOLDIER;
    	...
    	if (BattleLayer.city_id == 2) {
    		...
    		if (player_knight_type == KNIGHT_TYPE.KNIGHT_BOWMAN) {
    			battle_id = 20002;
    		}
    		else if (player_knight_type == KNIGHT_TYPE.KNIGHT_ENCHANTER) {
    			battle_id = 20003;
    		}
    	}
    ...
    public static InitArenaBattleInfo(self_direction:number = 1) {
    	if (self_direction > 0) {
    		self_direction = 1;
    	}
    	else {
    	...
    	BattleLayer.battle_type = BATTLE_TYPE.ARENA_BATTLE;
    ...
    public static AddPlayerKnightsToBattleForArena() {
    	var i = 0;
    	var span = 1;
    ...
    public static AddEnemyKnightsToBattleForArena(enemy_list) {
    	var i = 0;
    	var span = 1;
    	var direction = -1;
    ...
    public static AddArenaNpcKnightsToBattle(npc_list) {
    	var i = 4;
    	var span = -1;
    	var direction = -1;
    ...
    public static InitJobTeamBattleInfo(self_direction:number = 1) {
    	if (self_direction > 0) {
    		self_direction = 1;
    	}
    	else {
    	...
    	BattleLayer.battle_type = BATTLE_TYPE.JOB_TEAM_BATTLE;
    	BattleLayer.self_knight_direction = self_direction;
		BattleLayer.is_server_battle = false;
		BattleLayer.server_battle_result = 0;
    ...
    public static AddPlayerKnightsToBattleForJobTeam(members_list) {
    	var i = 0;
    	var span = 1;
    ...
    public static AddJobTeamNpcKnightsToBattle(npc_list) {
    	var i = 4;
    	var span = -1;
    	var direction = -1;
    ...
    private releaseKnightDragonRes(knight) {
	...
	private releaseAllDragonRes() {
   	...
   	private initBattleLayer() {
   		var battle_image_bg_name = "battle_bg_2_jpg";
   		if (BattleLayer.battle_type == BATTLE_TYPE.QUEST_BATTLE) {
   			var battle_id = Logic.getCityBattleId(BattleLayer.city_id, BattleLayer.city_battle_index);
    ...
    private endStartBattleAnim() {
    	if (BattleLayer.battle_type == BATTLE_TYPE.GUIDE_BATTLE1) {
    ...
    private HP_MASK_ANIM_TIME:number = 500;
    private runLeftTeamHpAnim() {
    ...
    private initBattleInforLayer() {
		...
		var knight_hp_start:number = 0;
        var knight_hp_span:number = 0;
        ...
        for (var i = 0; i < knights_len; i++) {
	        ...
	        // Add hp bar
	        var hp:egret.Bitmap = new egret.Bitmap();
        ...
        if (left_player_knight != null) {
			...
			var name_bg:egret.Bitmap = new egret.Bitmap();
			...
			var name_text:egret.TextField = new egret.TextField();
	        name_text.anchorX = 0;
    		name_text.anchorY = 0;
    		name_text.size = 25;
        ...
        var right_flag_name:string = "enemy_flag_bg_png";
		var left_flag_name:string = "self_flag_bg_png";
		if (BattleLayer.self_knight_direction == -1) {
        ...
        // Add right flag
		var right_flag:egret.Bitmap = new egret.Bitmap();
		...
		// Add left flag
		var left_flag:egret.Bitmap = new egret.Bitmap();
        ...
        // Add hp bg bar
	    var hp_bg_bar:egret.Bitmap = new egret.Bitmap();
        ...
        var right_hp_bar_name:string = "enemy_hp_large_png";
		var left_hp_bar_name:string = "self_hp_large_png";
		if (BattleLayer.self_knight_direction == -1) {
	    ...
	    var right_hp_anim:egret.Bitmap = new egret.Bitmap();
        ...
        var left_hp_anim:egret.Bitmap = new egret.Bitmap();
		...
		// Add right flag bar
		var right_flag_bar:egret.Bitmap = new egret.Bitmap();
		...
		// Add left flag bar
		var left_flag_bar:egret.Bitmap = new egret.Bitmap();
		...
		// Add vs image
		var vs_img:egret.Bitmap = new egret.Bitmap();
    ...
    private loadAllKnightsInBattle() {
    ...
    private onBattleLogicTimer( event ) {
    ...
    private static getBattleRandomInt(min, max) {
    ...
    private output_battle_info(strMsg) {
	...
	private refreshKnightPos(knight) {
	...
	private setKnightBaseData(knight, data) {
		knight.hp = data[1];
	...
	private setRemoteKnightBaseData(knight, data) {
		knight.hp = data[1];
		knight.hp_max = knight.hp;
	...
	private createOneEnchanter(data) {
		var knight = new Knight();
	...
	private createOneChevalier(data) {
		var knight = new Knight();
	...
	private createOneAssassinator(data) {
		var knight = new Knight();
	...
	private createOneLongBowman(data) {
		var knight = new Knight();
	...
	private createOneCrossBowman(data) {
		var knight = new Knight();
	...
	private createOneFireEnchanter(data) {
		var knight = new Knight();
	...
	private createOneIceEnchanter(data) {
		var knight = new Knight();
	...
	private createOneMinister(data) {
		var knight = new Knight();
	...
	public initKnightData(data) {
		var knight = null;
		if (data[0] == KNIGHT_TYPE.KNIGHT_SOLDIER) {
        	knight = this.createOneSoldier(data);
        }
        else if (data[0] == KNIGHT_TYPE.KNIGHT_BOWMAN) {
        	knight = this.createOneBowman(data);
        }
        else if (data[0] == KNIGHT_TYPE.KNIGHT_ENCHANTER) {
        	knight = this.createOneEnchanter(data);
        }
        else if (data[0] == KNIGHT_TYPE.KNIGHT_CHEVALIER) {
        	knight = this.createOneChevalier(data);
        }
        else if (data[0] == KNIGHT_TYPE.KNIGHT_FIGHTER) {
        	knight = this.createOneFighter(data);
        }
        else if (data[0] == KNIGHT_TYPE.KNIGHT_ASSASSINATOR) {
        	knight = this.createOneAssassinator(data);
        }
        else if (data[0] == KNIGHT_TYPE.KNIGHT_LONG_BOWMAN) {
        	knight = this.createOneLongBowman(data);
        }
        else if (data[0] == KNIGHT_TYPE.KNIGHT_CROSS_BOWMAN) {
        	knight = this.createOneCrossBowman(data);
        }
        else if (data[0] == KNIGHT_TYPE.KNIGHT_FIRE_ENCHANTER) {
        	knight = this.createOneFireEnchanter(data);
        }
        else if (data[0] == KNIGHT_TYPE.KNIGHT_ICE_ENCHANTER) {
        	knight = this.createOneIceEnchanter(data);
        }
        else if (data[0] == KNIGHT_TYPE.KNIGHT_MINISTER) {
	...
	private knight_start_hit(knight1, knight2)
	{
		// 自己进入近战攻击状态
		knight1.status = BATTLE_STATUS.HIT;
		...
		knight2.status = BATTLE_STATUS.HIT;
		...
		if (knight1.type == KNIGHT_TYPE.KNIGHT_FIGHTER && knight2.type == KNIGHT_TYPE.KNIGHT_FIGHTER) {
	...
	private showSunderArmorAnimation(knight) {
		if (knight.sunder_dragon == null) {
	...
	private bullet_disappear(bullet)
	{
	...
	// knight遇到bullet的远程攻击
	private knight_be_remote_hit(knight, bullet)
	{
		knight.status = BATTLE_STATUS.BE_REMOTE_HIT;
	...
	// knight即将死亡
	private knight_be_dying(knight)
	{
		if (this.IS_DEBUG_OUTPUT) {
	...
	private addBulletLayer(bullet) {
		...
		switch (bullet.type)
		{
			...
			case BULLET_TYPE.BULLET_LONG_BOW:
			{
			...
			case BULLET_TYPE.BULLET_CROSS_BOW:
			{
	...
	// knight进行远程攻击
	private knight_do_remote_hit(knight)
	{
		if (this.IS_DEBUG_OUTPUT) {
		...
		{
			case KNIGHT_TYPE.KNIGHT_BOWMAN:
				bullet.type = BULLET_TYPE.BULLET_BOW;
				break;

			case KNIGHT_TYPE.KNIGHT_ENCHANTER:
				bullet.type = BULLET_TYPE.BULLET_MAGIC;
				break;

			case KNIGHT_TYPE.KNIGHT_LONG_BOWMAN:
				bullet.type = BULLET_TYPE.BULLET_LONG_BOW;
				break;

			case KNIGHT_TYPE.KNIGHT_CROSS_BOWMAN:
				bullet.type = BULLET_TYPE.BULLET_CROSS_BOW;
				break;

			case KNIGHT_TYPE.KNIGHT_FIRE_ENCHANTER:
			{
				bullet.type = BULLET_TYPE.BULLET_FIRE;
			...
			case KNIGHT_TYPE.KNIGHT_ICE_ENCHANTER:
			{
				bullet.type = BULLET_TYPE.BULLET_ICE;
			...
			case KNIGHT_TYPE.KNIGHT_MINISTER:
				bullet.type = BULLET_TYPE.BULLET_SAN;
				break;
	...
	// knight进行远程治疗
	private knight_do_remote_recover(knight, knight_tobe_recover)
	{
	...
	// knight击退免疫
	private knight_back_immune(knight)
	{
	...
	private soldier_logic_tick(knight)
	{
		while (true)
		{
			/*
			 * MOVE：移动、HIT：近战攻击、BE_REMOTE_HIT：被远程打击、BE_HIT：计算受攻击伤害
			 * DEAD：死亡、BACKING：击退中
			 */
			switch (knight.status)
			{
				case BATTLE_STATUS.MOVE:
				{
					...
					if (knight.status != BATTLE_STATUS.MOVE)
						break;
					...
					if (knight.move_direction == 1)
					{
						...
						for (var i = 0; i < iEnemyLen; i++)
						{
							...
							if (distance < min_enemy_distance && this.right_knights_list[i].status != BATTLE_STATUS.DEAD)
								min_enemy_distance = distance;

							if (knight.pos_x >= this.right_knights_list[i].pos_x 
									&& this.right_knights_list[i].status == BATTLE_STATUS.MOVE)
							{
					...
					else
					{
						...
						for (var i = 0; i < iEnemyLen; i++)
						{
							...
							if (distance < min_enemy_distance && this.left_knights_list[i].status != BATTLE_STATUS.DEAD)
								min_enemy_distance = distance;

							if (knight.pos_x <= this.left_knights_list[i].pos_x
									&& this.left_knights_list[i].status == BATTLE_STATUS.MOVE)
							{
					...
					if (min_enemy_distance <= knight.hit_show_distance 
						&& knight.hit_start_time + knight.hit_show_time < this.TIME)
					{
						// 开始攻击前摇动画
					...
					if (knight.knight_dragon.animation.isPlaying == false 
        				|| (knight.knight_dragon.animation.lastAnimationName != "benpao" && knight.knight_dragon.animation.lastAnimationName != "gongji")) {
				...
				case BATTLE_STATUS.BACKING:
				{
					// 1. BACKING --> MOVE
					if (knight.self_back_distance <= 0 || knight.pos_x >= this.RIGHT_SIDE
							|| knight.pos_x <= this.LEFT_SIDE)
					{
						knight.self_back_distance = 0;
						knight.status = BATTLE_STATUS.MOVE;
					...
					if (knight.status != BATTLE_STATUS.BACKING)
						break;
					...
					if (knight.knight_dragon.animation.isPlaying == true 
        				&& (knight.knight_dragon.animation.lastAnimationName != "aida" 
        					&& knight.knight_dragon.animation.lastAnimationName != "gongji")) {
        				knight.knight_dragon.animation.gotoAndPlay("aida");
        			}
					else if (knight.knight_dragon.animation.isPlaying == false 
        				&& knight.knight_dragon.animation.lastAnimationName != "aida") {
        				knight.knight_dragon.animation.gotoAndPlay("aida");
        			}
				...
				case BATTLE_STATUS.HIT:
				{
					// HIT --> BE_HIT
					knight.status = BATTLE_STATUS.BE_HIT;
					
					this.knight_back_immune(knight);
					break;
				}

				case BATTLE_STATUS.BE_REMOTE_HIT:
				{
				...
				case BATTLE_STATUS.BE_HIT:
				{
					...
					if (knight.hp <= 0)
					{
						// 1. BE_HIT --> DEAD
						knight.hp = 0;
						knight.status = BATTLE_STATUS.DEAD;
					}
					else
					{
						// 2. BE_HIT --> BACKING
						knight.status = BATTLE_STATUS.BACKING;
	...
	private bowman_logic_tick(knight)
	{
		while (true)
		{
			/*
			...
			switch (knight.status)
			{
				case BATTLE_STATUS.MOVE:
				{
						...
						&& knight.hit_start_time + knight.hit_show_time < this.TIME)
					{
						...
						if (knight.knight_dragon.animation.isPlaying == false 
	        				|| knight.knight_dragon.animation.lastAnimationName != "jinzhangongji") {
					...
					else
					{
						...
						{	
							...
							{
								// (远程攻击距离内没有敌人，并且上次没有成功攻击，移动)
							}
						}
						// 6. Remote hit showing 1 (远程攻击前摇1中)
						else if (knight.remote_hit_cd_start_time + knight.remote_hit_cd > this.TIME) {
							if (knight.knight_dragon.animation.isPlaying == false 
		        				|| (knight.knight_dragon.animation.lastAnimationName != "gongjiq" && knight.knight_dragon.animation.lastAnimationName != "gongjih")) {
						...
						// 7. Start remote hit showing 2 (攻击前摇1结束，开始远程攻击前摇2)
						...
						// 8. Remote hit showing 2 (远程攻击前摇2中)
						...
						// 9. Remote hit (远程攻击)
						else //if (knight.remote_hit_start_time > 0)
						{
        			...
        			if (knight.knight_dragon.animation.isPlaying == false 
        				|| (knight.knight_dragon.animation.lastAnimationName != "benpao" && knight.knight_dragon.animation.lastAnimationName != "jinzhangongji")) {
        				knight.knight_dragon.animation.gotoAndPlay("benpao");
        			}
	...
	private enchanter_logic_tick(knight)
	{
		while (true)
		{
			/*
	...
	private minister_logic_tick(knight)
	{
		while (true)
		{
			/*
				...
				case BATTLE_STATUS.MOVE:
				{
					...
					else
					{
						...
						if (knight.remote_hit_start_time < 0 && knight.remote_hit_cd_start_time < 0)
						{
							...
							if (knight.move_direction == 1)
							{
								...
								for (var i = 0; i < iFriendLen; i++)
								{
									...
									if (distance < min_hurt_friend_distance 
											&& this.left_knights_list[i].status != BATTLE_STATUS.DEAD
											&& this.left_knights_list[i].hp < this.left_knights_list[i].hp_max)
							...
							else
							{
								...
								for (var i = 0; i < iFriendLen; i++)
								{
									...
									if (distance < min_hurt_friend_distance 
											&& this.right_knights_list[i].status != BATTLE_STATUS.DEAD
											&& this.right_knights_list[i].hp < this.right_knights_list[i].hp_max)
										min_hurt_friend_distance = distance;
								}
							}

							// 5. Start remote hit showing 1 (治疗距离内有队友、攻击距离内有人、连续攻击中，开始远程攻击前摇1)
							if (min_hurt_friend_distance <= knight.remote_recover_distance
									|| min_enemy_distance <= knight.remote_hit_distance 
									|| knight.remote_hit_combo > 0) 
							{
								knight.remote_hit_cd_start_time = this.TIME;
								
								if (knight.knight_dragon.animation.isPlaying == false 
			        				|| (knight.knight_dragon.animation.lastAnimationName != "gongjijiaxue" 
			        					&& knight.knight_dragon.animation.lastAnimationName != "gongji")) {
			        				knight.knight_dragon.animation.gotoAndPlay("gongjiq");
			        			}
							...
							{
								// (治疗距离内没有队友，且远程攻击距离内没有敌人，且上次没有成功攻击，移动)
							}
						...
						// 6. Remote hit showing 1 (远程攻击前摇1中)
							...
							if (knight.knight_dragon.animation.isPlaying == false 
		        				|| (knight.knight_dragon.animation.lastAnimationName != "gongjiq"
		        					&& knight.knight_dragon.animation.lastAnimationName != "gongjijiaxue"
		        					&& knight.knight_dragon.animation.lastAnimationName != "gongji")) {
		        				knight.knight_dragon.animation.gotoAndPlay("gongjiq");
		        			}
						...
						// 9. Remote hit (远程攻击 或者 远程治疗，优先治疗)
						else //if (knight.remote_hit_start_time > 0)
						{
							...
							for (var i = 0; i < iFriendLen; i++)
							{
								...
								if (friends_list[i].status != BATTLE_STATUS.DEAD
										&& iMaxLostHp < lost_hp)
								{
									iMaxLostHp = lost_hp;
	 ...
	 */
	private knight_logic_tick(knight)
	{
		switch (knight.type)
		{
			case KNIGHT_TYPE.KNIGHT_SOLDIER:
	 ...
	 */
	private bullet_logic_tick(bullet)
	{
		// 子弹只要还存在就继续移动
	 ...
	 */
	private battle_logic_tick():boolean
	{
		if (this.IS_DEBUG_OUTPUT) {
		...
		for (var i = 0; i < iLeftLen; i++)
		{
			if (this.left_knights_list[i].status != BATTLE_STATUS.DEAD)
			{
				left_all_dead = false;
				break;
			}
		...
		for (var i = 0; i < iRightLen; i++)
		{
			if (this.right_knights_list[i].status != BATTLE_STATUS.DEAD)
			{
		...
		for (var i = 0; i < iLeftLen; i++)
		{
			if (this.left_knights_list[i].status == BATTLE_STATUS.HIT)
			{
				this.knight_logic_tick(this.left_knights_list[i]);
			}
		}
	...
	private showBullteMoveOutAnim(bullet)
	{
		var end_pos_x = -50;
		if (bullet.move_direction == 1) {
	...
	private showKnightMoveOutAnim(knight)
	{
		knight.knight_dragon.animation.gotoAndPlay("benpao");
	...
	// 得到队伍剩余血量百分比
	public getLeftSideHpInfo() {
		var iLeftLen:number = this.left_knights_list.length;
		var hp:number = 0;
		var max_hp:number = 0;
		for (var i = 0; i < iLeftLen; i++)
		{
	...
	public getRightSideHpInfo() {
		var iRightLen:number = this.right_knights_list.length;
		var hp:number = 0;
		var max_hp:number = 0;
		for (var i = 0; i < iRightLen; i++)
		{
