class TrieNode {
    ...
    public AddKey(word:string) {
        var length = word.length;
        if (length <= 0) {
            return;
        }

        var pNode:TrieNode = this;
        for (var i = 0; i < length; i++) {
            var k = word[i];
            var pSubNode:TrieNode = null;
...
class BadWords {
    ...
    public static BadwordFilter(source:string):string {
    ...
    public static HasAnyBadword(source:string):boolean {
        BadWords.initBadWordsList();
