module Astar {
    export enum listMode{list_None,list_Open,list_Close};
    export class ANode {
        ...
        public checkIsNeedChgParent(version: number, checkparent: ANode) {
            ...
            var ng: number = (Math.abs(this.x - checkparent.x) + Math.abs(this.y - checkparent.y)) == 2 ? Maze.STEP : Maze.OBLIQUE;
            var newg = ng + checkparent.getG(version);
            if (newg > this._g) return;
            this._g = newg;
