...
class Utils {
    ...
    public static createBackgroundTexture( step:number ) {
        ...
        if ( !RES.isGroupLoaded( "bg_" + bg_name ) ) {
            ...
            var group_name = "bg_" + bg_name
            if ( RES.createGroup( group_name, [bg_name, bg_name+"a"] ) ) {
                //console.log( "------ loadgroup", group_name )
                RES.loadGroup( group_name )
            }
        }
        ...
        if ( !RES.isGroupLoaded( next_group_name ) ) {
            if ( RES.createGroup( next_group_name, [next_bg_name, next_bg_name+"a"] ) ) {
                //console.log( "====== loadgroup", next_group_name )
...
class Net {
    ...
    public static getOfflineMoney() {
        var s = (G.account_name || Utils.getUUID())
        Net.postData( 'data=' + '{"uuid":"' + s + '", "command":"OFFLINE_MONEY"}', Net.onReceiveGetOfflineMoney )
    }

    public static onReceiveGetOfflineMoney( event:egret.Event ) {
        var loader:egret.URLLoader = <egret.URLLoader> event.target;
        var data:egret.URLVariables = loader.data;
        var m = JSON.parse( data.toString() )
        console.log( "onReceiveGetOfflineMoney", m.result, m.offline_money, m.offline_step )
        if ( m.result != 0 ) {
            console.log( "result", m.result )
            return
        }
    ...
    public static collectOfflineMoney() {
        var s = (G.account_name || Utils.getUUID())
        Net.postData( 'data=' + '{"uuid":"' + s + '", "command":"COLLECT_OFFLINE_MONEY"}', Net.onReceiveCollectOfflineMoney )
    }

    public static onReceiveCollectOfflineMoney( event ) {
        var loader:egret.URLLoader = <egret.URLLoader> event.target;
        var data:egret.URLVariables = loader.data;
        var m = JSON.parse( data.toString() )
        console.log( "onReceiveGetOfflineMoney", m.result, m.offline_money, m.offline_step )
        ...
        Logic.offline_step = Number(m["offline_step"])
        if ( GameView.instance() != null ) {
            GameView.instance().receiveOfflineMoney()
        }
    }
