class PanelMain extends egret.gui.Panel{
    ...
    public main_difficulties_award:egret.gui.UIAsset;
    ...
    private m_item_player:ItemPlayer;
    private m_item_heroes = [];
    ...
    public onCreationComplete( evt ) {
        ...
        var tw = egret.Tween.get( this.btn_main_speedup, {loop:true} )
        tw.to( {scaleX:0.95, scaleY:0.95, x:this.btn_main_speedup.x-20}, 80 ).to( {scaleX:1, scaleY:1, x:this.btn_main_speedup.x}, 80 ).wait( 1500 )

        this.icon_damage100.addEventListener( egret.TouchEvent.TOUCH_TAP, this.onBtnSpeedUp, this )
        this.icon_money100.addEventListener( egret.TouchEvent.TOUCH_TAP, this.onBtnSpeedUp, this )
    }
    ...
    public refreshDifficultyAward() {
        if ( !this.btn_main_difficulties.visible ) {
            egret.Tween.removeTweens( this.main_difficulties_award )
            this.main_difficulties_award.visible = false
            return
        }
        if ( Logic.getDifficultyAward() != null ) {
            this.main_difficulties_award.visible = true
            var tw = egret.Tween.get( this.main_difficulties_award, {loop:true} )
            tw.to( {alpha:0.4}, 500 ).to( {alpha:1}, 500 ).wait( 200 )
        }
        else {
            egret.Tween.removeTweens( this.main_difficulties_award )
            this.main_difficulties_award.visible = false
    ...
    public showNotOpenGuide( type, n ) {
        var gameView = GameView.instance()
        if ( gameView  == null ) {
            return
        }
        if ( type == 1 ) {
            gameView.showToast( "该系统在" + n + "级开启", 1600 )
        }
        else if ( type == 2 ) {
            gameView.showToast( "该系统第" + n + "回后开启", 1600 )
        }
        else if ( type == 3 ) {
            gameView.showToast( "该系统在消灭" + n + "只妖怪后开启", 1600 )
        }
    }


    public checkNotOpen( btn ) {
        var n_reset = Stat.getData( ST.RESET )
        if ( btn == this.btn_main1 ) {
            if (n_reset == 0 && Logic.step == 1 && ( Logic.sub_step < 5 && Logic.sub_step != -1 ) ) {
                this.showNotOpenGuide( 3, 5 )
                return true
            }
        }
        else if ( btn == this.btn_main2 ) {
            if (n_reset == 0 && Logic.step == 1 && (Logic.sub_step < 5 && Logic.sub_step != -1) ) {
                this.showNotOpenGuide( 3, 5 )
                return true
            }
        }
        else if ( btn == this.btn_main3 ) {
            if (n_reset == 0 && Logic.step == 1 && (Logic.sub_step < 8 && Logic.sub_step != -1)) {
                this.showNotOpenGuide( 3, 8 )
                return true
            }
        }
        else if ( btn == this.btn_main4 ) {
            if (n_reset == 0 && Logic.step == 1 && (Logic.sub_step < 8 && Logic.sub_step != -1)) {
                this.showNotOpenGuide( 3, 8 )
                return true
            }
        }
        else if ( btn == this.btn_main_difficulties ) {
            if (Logic.step < 20 ) {
                this.showNotOpenGuide( 2, 20 )
                return true
            }
        }
        return false
    ...
    public refreshAllAboutDamage() {
        this.refreshDamageInfo()
        if ( this.m_item_player ) {
            this.m_item_player.refreshPlayerInfo()
        }
        for ( var i=0; i<this.m_item_heroes.length; ++i ) {
            var item_hero:ItemHero = this.m_item_heroes[i]
            item_hero.refreshHeroInfo()
        }
    }
    ...
    public reviveHeroItem( hero_id ) {
        for ( var i=0; i<this.m_item_heroes.length; ++i ) {
            if ( this.m_item_heroes[i].m_iHeroID == hero_id ) {
                this.m_item_heroes[i].onRevive()
                break
            }
        }
    }
    ...
    public protectHeroTick() {
        ...
        if (iLeftTime > 0) {
            ...
            if ( G.isLightBackground() ) {
                this.protect_hero_tick.textColor = 0x000000
            }
            else {
                this.protect_hero_tick.textColor = 0xFFFFFF
            }
        } else {
    ...
    private addPlayerItem() {
        ...
        this.m_item_player = uiPlayerItem
    }
    ...
    private addOneHeroItem(index:number) : boolean {
        ...
        this.m_item_heroes.push( uiHeroItem )
    ...
    public refreshSpeedUp() {
        if ( Logic.getSpeedUpTime() > 0 ) {
            ...
            if ( G.isLightBackground() ) {
                ...
                this.label_speedup_time.textColor = 0xFFFFFF
            }
            this.btn_main_speedup.visible = false
        }
        else {
            this.label_speedup_time.visible = false
            this.icon_damage100.visible = false
            this.icon_money100.visible = false
            this.btn_main_speedup.visible = true
        }
    }
}
