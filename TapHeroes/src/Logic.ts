...
class StatisticType {
    ...
    public static SPEED_UP = 17; // 加速礼包次数
}

var AchivementDiamonds = [5, 15, 25, 50, 100]
...
class Logic {
    ...
    public static difficulty_award = 0;
    ...
    public static diamond_receive_offline_money = false;
    ...
    public static dumpDataShort() {
        ...
        data.push( Logic.difficulty_award )
        data.push( Logic.speedup_time )
        return data
    }
    ...
    public static loadDataShort(data, server=true) {
        ...
        Logic.difficulty_award = data.splice(0,1)[0] || 0
        Logic.speedup_time = data.splice(0,1)[0] || 0
    }
    ...
    public static getOfflineMoney() {
        ...
        var diamond_times = 1
        if ( Logic.diamond_receive_offline_money ) {
            diamond_times = Logic.getOfflineMoneyDiamondTimes()
            if ( ! Logic.costDiamond( Logic.getOfflineMoneyCostDiamond() ) ) {
                return 0
            }
        }
        return base * times * diamond_times
    }

    public static getOfflineMoneyDiamondTimes() {
        return 4
    }

    public static getOfflineMoneyCostDiamond() {
        var times = Logic.offline_money
        var d = times / 43.2
        d = Math.round( d )
        if ( d < 10 ) {
    ...
    public static getSpeedUpTotalMoney( step ) {
        var m = Logic._getBaseMoney( step )
        m *= 9999
        return m
    }
    ...
    public static speedUp() {
        ...
        var m = Logic.getSpeedUpTotalMoney( Logic.step )
        Stat.addData( ST.SPEED_UP, 1 );
        return [0, m]
    }
    ...
    public static calcDifficultyMonsterMaxHP() {
        //29*1.57的N-1次方
        var line = U.getLine( "difficulty_list", Logic.difficulty_challenge_id )
        var step = line[ difficulty_list_step ]
        var hp = 29 * Math.pow( 1.57, step )
        if ( Logic.monster.type == "boss" ) {
            var n = 8
            hp  = hp * n;
        }
    ...
    public static getPlayerTitle() {
        var names = ["石猴", "猴大王", "弼马温", "美猴王", "悟空", "大师兄", "孙行者", "心猿", "金公", "齐天大圣", "斗战胜佛"]
        var l = [1,4,7,11,16,21,26,31,41,51,71, 1000]
        var n = Logic.difficulty_passed_id
        var s = names[0]
    ...
    public static getDifficultyAward() {
        if ( Logic.difficulty_award >= Logic.difficulty_passed_id-1 ) {
            return null
        }
        var l = [0,0,0]
        for ( var i=Logic.difficulty_award+1; i<Logic.difficulty_passed_id; ++i ) {
            var ll = Logic.getDifficultyAwardById( i )
            l[0] += ll[0]
            l[1] += ll[1]
            l[2] += ll[2]
        }
    ...
    public static collectDifficultyAward() {
        ...
        Logic.addMoney( l[0] )
        Logic.addPeach( l[1] )
        Logic.addDiamond( l[2] )
        Logic.difficulty_award = Logic.difficulty_passed_id-1
    }
    ...
    public static getCleanSkillCDDiamond() {
        var m = 0
        for ( var i=0; i<Logic.player.skills.length; ++i ) {
            var t = Logic.player.skills[i].cd_time - U.time()
            if ( t > m ) {
                m = t
            }
        }
