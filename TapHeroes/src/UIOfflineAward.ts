class OfflineAwardDialog extends egret.gui.Panel{
    ...
    public onCreationComplete( evt ) {
        ...
        this.label_multi.text = "挂机" + Logic.getOfflineMoneyDiamondTimes() + "倍奖励"
        this.label_money.text = U.bigNumber( Logic.getOfflineMoney() )
        this.label_money_multi.text = U.bigNumber( Logic.getOfflineMoney() * Logic.getOfflineMoneyDiamondTimes() )
    }
    ...
    public onBtnReceive( evt ) {
        ...
        Logic.diamond_receive_offline_money = false
        Net.collectOfflineMoney()
        this._close()
    }
    ...
    public onBtnDiamondReceive( evt ) {
        ...
        var gameView = GameView.instance()
        if ( Logic.diamond < Logic.getOfflineMoneyCostDiamond() ) {
            this._close()
            gameView.showLowDiamondHint();
            return
        }
        Logic.diamond_receive_offline_money = true
        Net.collectOfflineMoney()
        this._close()
    }
